...
cc -std=c89 -pedantic-errors -Wall -Wextra -Werror ${WENA_TEST_CFLAGS:-} \
 ...
 "$root_dir/server/sqlite_persistence.c" "$root_dir/server/mutations/list_archive.c" "$root_dir/server/mutations/board_settings.c" "$root_dir/server/mutations/checklist_order.c" "$root_dir/models/checklist.c" "$root_dir/models/checklist_item.c" \
