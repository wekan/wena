...
test_dir="${TMPDIR:-/tmp}/wena-card-archive-state-$$"
mkdir -p "$test_dir"
trap 'rm -rf "$test_dir"' EXIT HUP INT TERM
cc -std=c89 -pedantic-errors -Wall -Wextra -Werror \
 "$root_dir/tests/card_archive_state_test.c" "$root_dir/client/features/hierarchy_mutation.c" "$root_dir/client/features/card_mutation.c" "$root_dir/models/card_order.c" \
