...
int main(void)
{
    ...
    assert(wena_table_init(&state,2));view.row_count=5;view.windowed=1;
    view.available_first=0;view.available_count=2;
    result=frame(&ctx,&state,&view);assert(result.valid&&!result.needs_rows&&calls==2);
    result=click(&ctx,&state,&view,"Next Page");
    assert(result.valid&&result.page_changed&&result.needs_rows&&!calls&&!result.action);
    assert(result.first_row==2&&result.row_count==2);center(&ctx,"Loading, please wait.");
    view.available_first=2;view.available_count=2;
    result=frame(&ctx,&state,&view);assert(!result.needs_rows&&calls==2&&rows[0]==2);
    view.available_first=3;result=frame(&ctx,&state,&view);assert(result.needs_rows&&!calls);
    view.available_first=2;view.available_count=1;
    result=frame(&ctx,&state,&view);assert(result.needs_rows&&!calls);
    view.available_first=(size_t)-1;view.available_count=(size_t)-1;
    result=frame(&ctx,&state,&view);assert(result.needs_rows&&!calls);
    view.available_first=0;view.available_count=0;view.render_row=NULL;
    result=frame(&ctx,&state,&view);assert(result.valid&&result.needs_rows&&!calls);
    view.row_count=0;result=frame(&ctx,&state,&view);assert(!result.needs_rows&&!calls&&!state.page);
    view.windowed=0;view.render_row=virtual_row;view.row_count=256;
