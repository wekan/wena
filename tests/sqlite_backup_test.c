...
static void schema_rejections(sqlite3 *db, const char *path, unsigned long *room)
{
    ...
    for (index = 0; index < sizeof(invalid) / sizeof(invalid[0]); ++index) {
        ...
        assert(!wena_sqlite_backup_create(db, path, space, room));
