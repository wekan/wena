...
static void open_move(struct nk_context *ctx, WenaChecklistsState *state, WenaCard *cards)
{
    ...
    click(ctx, state, cards, "Edit"); character(ctx, state, cards, 0);
    strcpy(state->input, "Unsaved rename"); state->length = (int)strlen(state->input);
    click(ctx, state, cards, "Destination"); character(ctx, state, cards, 0);
    assert(state->action == WENA_CHECKLIST_MOVE_ITEM && !strcmp(state->input, "Completed"));
}
static void destination(struct nk_context *ctx, WenaChecklistsState *state, WenaCard *cards,
    const char *card_label, const char *list_label)
{
    click(ctx, state, cards, "Cards"); character(ctx, state, cards, 0);
    click(ctx, state, cards, card_label); character(ctx, state, cards, 0);
    assert(state->target_card_version && state->target_snapshot);
    click(ctx, state, cards, "Checklist"); character(ctx, state, cards, 0);
    click(ctx, state, cards, list_label); character(ctx, state, cards, 0);
    assert(state->target_checklist_version);
}
int main(int argc, char **argv)
{
    ...
    sql(db, "INSERT INTO checklists(id,board_id,card_id,title,position) VALUES"
        "('cl','b','src','Tasks',8),('keep','b','src','Tasks',9),"
        "('dest','b','dst','Tasks',0),('dest2','b','dst','Tasks',1);"
    ...
    destination(&ctx, &state, cards, "Same title [dst]", "Tasks [dest2]");
    ...
    assert(!writes && state.action == WENA_CHECKLIST_MOVE_ITEM);
    click(&ctx, &state, cards, "Cancel"); assert(!state.action && !state.target_snapshot && !writes);
    open_move(&ctx, &state, cards);
    destination(&ctx, &state, cards, "Source [src]", "Tasks [keep]");
    key(&ctx, &state, cards, NK_KEY_TEXT_RESET_MODE, 1);
    assert(!state.visible && !state.snapshot && !state.target_snapshot && !writes);
    key(&ctx, &state, cards, NK_KEY_TEXT_RESET_MODE, 0);
    assert(wena_checklists_open(&state, cards));
    open_move(&ctx, &state, cards);
    destination(&ctx, &state, cards, "Source [src]", "Tasks [keep]");
    click(&ctx, &state, cards, "Save");
    assert(!state.error && !state.action && !state.target_snapshot && state.snapshot->card_version == 2);
    assert(!strcmp(state.snapshot->items[0].checklist_id, "keep"));
    open_move(&ctx, &state, cards);
    destination(&ctx, &state, cards, "Same title [dst]", "Tasks [dest2]");
    /* Destination selection captures both revisions. A concurrent parent edit
     * refuses even though the selected source item did not change. */
    sql(db, "UPDATE checklists SET version=version+1 WHERE id='dest2';"
        "UPDATE cards SET version=version+1 WHERE id='dst'");
    click(&ctx, &state, cards, "Save");
    assert(state.error && state.action == WENA_CHECKLIST_MOVE_ITEM && state.target_card_version == 1);
    assert(state.target_checklist_version == 1 && state.snapshot->item_count == 1);
    click(&ctx, &state, cards, "Cancel");
    open_move(&ctx, &state, cards);
    destination(&ctx, &state, cards, "Same title [dst]", "Tasks [dest2]");
    assert(state.target_card_version == 2 && state.target_checklist_version == 2);
    /* Switching to a card without checklists drops the previous checklist ID. */
    click(&ctx, &state, cards, "Same title [dst]"); character(&ctx, &state, cards, 0);
    click(&ctx, &state, cards, "Same title [another]"); character(&ctx, &state, cards, 0);
    previous = writes; click(&ctx, &state, cards, "Save");
    assert(state.error && !state.target_checklist_id[0] && writes == previous);
    click(&ctx, &state, cards, "Cancel");
    open_move(&ctx, &state, cards);
    destination(&ctx, &state, cards, "Same title [dst]", "Tasks [dest2]");
    ...
    character(&ctx, &state, cards, 0);
    click(&ctx, &state, cards, "Save");
    assert(state.error && !state.target_snapshot && !state.target_checklist_version && writes == previous);
    cards[1].archived = 0;
    destination(&ctx, &state, cards, "Same title [dst]", "Tasks [dest2]");
    sql(db, "CREATE TRIGGER fail BEFORE INSERT ON idempotency_keys BEGIN SELECT RAISE(ABORT,'late'); END");
    click(&ctx, &state, cards, "Save"); assert(state.error && state.action == WENA_CHECKLIST_MOVE_ITEM);
    assert(state.snapshot->item_count == 1 && state.target_snapshot);
    sql(db, "DROP TRIGGER fail");
    fail_reload = 1; click(&ctx, &state, cards, "Save");
    assert(state.needs_refresh && state.error && !state.action && !state.target_snapshot);
    ...
    assert(!state.needs_refresh && !state.error && state.snapshot->checklist_count == 2 &&
    ...
    assert(snapshot->item_count == 1 && snapshot->items[0].is_finished &&
        !strcmp(snapshot->items[0].checklist_id, "dest2") && snapshot->items[0].position == 0);
    wena_checklist_snapshot_free(snapshot); nk_free(&ctx); assert(sqlite3_close(db) == SQLITE_OK);
    puts("Real Nuklear/SQLite item transfer: exact parents, same/cross-card, cancellation, stale, refresh and reopen passed");
    return 0;
}
