...
test_dir=$(mktemp -d "${TMPDIR:-/tmp}/wena-schema-v6-XXXXXX")
...
cc -std=c89 -pedantic-errors -Wall -Wextra -Werror \
 "$root_dir/tests/sqlite_schema_v6_test.c" "$root_dir/server/sqlite_storage.c" \
 "$root_dir/server/sqlite_backup.c" "$root_dir/server/sqlite_restore.c" \
 "$root_dir/server/sha256.c" -lsqlite3 -o "$test_dir/test"
"$test_dir/test" "$test_dir/v1.sql" "$test_dir/v2.sql" "$test_dir/v3.sql" "$test_dir/v4.sql" "$test_dir/v5.sql" "$test_dir/v6.sql" "$test_dir"
