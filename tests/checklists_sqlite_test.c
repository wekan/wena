...
int main(int argc,char**argv)
{
 ...
 frame(&s,&c,"Checklist Actions",NULL);choose(&s,&c,"No");frame(&s,&c,"Save",NULL);assert(!s.error&&s.snapshot->checklists[0].show_on_minicard==WENA_CHECKLIST_MINICARD_HIDE);
 wena_checklists_close(&s);assert(wena_checklists_open(&s,&c));assert(s.snapshot->checklists[0].show_on_minicard==WENA_CHECKLIST_MINICARD_HIDE);
 frame(&s,&c,"Checklist Actions",NULL);choose(&s,&c,"Default");frame(&s,&c,"Save",NULL);assert(!s.error&&s.snapshot->checklists[0].show_on_minicard==WENA_CHECKLIST_MINICARD_INHERIT);
