...
in a reusable pure model, used by the Move dialog, card dragging and the guarded
mutation adapter;
...
- [_] Possible to drag drop same way like Meteor 3 WeKan
  ...
  - [x] Add the native exact-insertion mutation adapter using the same ordered
    column snapshots and fingerprints, including empty destinations. Prepare the
