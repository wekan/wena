...
void wena_card_move_close(WenaCardMoveState *state)
{
    ...
    void *context;
    if (state == NULL) return;
    load = state->load; apply = state->apply; context = state->context;
...
static void option_label(void *data, int index, const char **text)
{
    ...
    if (id == NULL) strcpy(options->label, "[?]");
    ...
    char *target)
{
    ...
    if (count == 0) { nk_label(context, "[?]", NK_TEXT_LEFT); return; }
    ...
    const WenaBoardLayout *layout, float width, float height)
{
    ...
    close_requested = 0;
    if (nk_begin(context, "Move card", nk_rect(width * 0.5f, 0,
        ...
        if (nk_button_label(context, wena_ui_control_text(WENA_UI_SAVE))) {
            if (find_lane(layout, state->target_swimlane_id) != NULL &&
                ...
                state->apply != NULL && state->apply(state->context, state->board_id,
                    state->card_id, state->version, state->target_list_id,
                    state->target_swimlane_id)) close_requested = 1;
            else state->error = 1;
        }
