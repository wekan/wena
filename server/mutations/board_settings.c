...
static int stored_flag(sqlite3_stmt *statement,int column,int default_value,int *value)
{
    if (sqlite3_column_type(statement,column+2)==SQLITE_NULL) { *value=default_value; return 1; }
...
static int read_settings(sqlite3 *database,const char *board_id,unsigned long expected,
    int *stored,int *stored_contents)
{
    sqlite3_stmt *statement;
    int valid;
    if (sqlite3_prepare_v2(database,
        "SELECT b.version,s.show_checklist_count,m.show_checklists,s.board_id,m.board_id FROM boards b "
        "LEFT JOIN board_settings s ON s.board_id=b.id "
        "LEFT JOIN board_minicard_settings m ON m.board_id=b.id WHERE b.id=?1",-1,
        &statement,NULL)!=SQLITE_OK) return 0;
    valid=sqlite3_bind_text(statement,1,board_id,-1,SQLITE_TRANSIENT)==SQLITE_OK &&
        sqlite3_step(statement)==SQLITE_ROW &&
        sqlite3_column_type(statement,0)==SQLITE_INTEGER &&
        sqlite3_column_int64(statement,0)==(sqlite3_int64)expected &&
        stored_flag(statement,1,0,stored) && stored_flag(statement,2,1,stored_contents);
    if (sqlite3_finalize(statement)!=SQLITE_OK) valid=0;
    return valid;
}
    ...
    unsigned long *result_version)
{
    ...
    int requested,stored,requested_contents,stored_contents,valid,display;
    ...
    if (!read_settings(database,board_id,expected,&stored,&stored_contents)) return 0;
    if (!display) requested_contents=stored_contents;
    if (stored==requested && stored_contents==requested_contents) {*result_version=expected;return 2;}
    ...
    if (sqlite3_finalize(statement)!=SQLITE_OK) valid=0;
    if (!valid) return 0;
    if (!read_settings(database,board_id,expected+1UL,&stored,&stored_contents) ||
        stored!=requested || stored_contents!=requested_contents) return 0;
