...
int wena_sqlite_hierarchy_color_read(sqlite3 *db,const char *board,const char *id,
    int lists,unsigned long expected,char color[WENA_COLOR_CAPACITY])
{
    sqlite3_stmt *s;char sql[512],candidate[WENA_COLOR_CAPACITY];
    const unsigned char *text,*scope;int ok,bytes;const char *parent,*table,*key;
    if(!db||!color||(lists!=0&&lists!=1)||!expected||expected>WENA_VERSION_READ_MAX||
        !wena_model_identifier_valid(board)||!wena_model_identifier_valid(id))return 0;
    parent=lists?"lists":"swimlanes";table=lists?"list_colors":"swimlane_colors";key=lists?"list_id":"swimlane_id";
    if(wena_sqlite_optional_table(db,table,11)!=1)return 0;
