...
const char *wena_legacy_html4_capability_script(void)
{
    static const char *const parts[] = {
        ...
        "function visibleState(doc){var nodes=doc.getElementsByClassName('wena-visible-region'),visible={},i,name,version,allowed=/^((board|sidebar|card-details)|(card|list|swimlane)-[A-Za-z0-9_-]+)$/;for(i=0;i<nodes.length;i+=1){name=nodes[i].getAttribute('data-wena-region');version=safeNumber(nodes[i].getAttribute('data-wena-version'));",
        "if(!allowed.test(name)||nodes[i].id!=='wena-region-'+name||Object.prototype.hasOwnProperty.call(visible,name)){throw new Error('invalid visible regions');}visible[name]={elementId:nodes[i].id,version:version};}if(nodes.length===0||nodes.length>8){throw new Error('no visible regions');}return visible;}\n",
        ...
        "controller=new window.AbortController();version=state.nextRequestVersion;if(typeof version!=='number'||version<=state.lastRequestVersion){restore();return Promise.reject(new Error('invalid request sequence'));}state.inFlight=true;promise=Promise.resolve().then(function(){if(controller.signal.aborted){throw new Error('enhancement aborted');}body=new window.URLSearchParams(new window.FormData(form));",
        "return window.fetch(action,{method:'POST',credentials:'same-origin',redirect:'error',headers:{'Accept':'application/vnd.wena.regions-v1','Content-Type':'application/x-www-form-urlencoded;charset=UTF-8','X-Wena-Request-Version':String(version)},body:body.toString(),signal:controller.signal});})",
        ...
        "function enhance(doc){var state;try{state={lastRequestVersion:0,nextRequestVersion:1,inFlight:false,visible:visibleState(doc)};}catch(ignore){restore();return false;}if(!activate(doc)){return false;}if(!bindDragDrop(doc,state,5000)){restore();return false;}return true;}\n",
        "window.WenaLegacyEnhancement={activate:activate,restore:restore,guard:guard,parseRegions:parseRegions,applyRegions:applyRegions,postForm:postForm,bindDragDrop:bindDragDrop,visibleState:visibleState};\nwindow.addEventListener('wena:enhancement-failed',restore);\n",
